...
async function recreateFarmDilemma() {
  try {
    // 1. 기존 농장 딜레마 삭제
    console.log('🗑️  Deleting old farm dilemma...')
    await prisma.dilemma.delete({
      where: { id: 'cmgzae36r0000hbb74fb5wv25' }
    }).catch(() => console.log('   (Already deleted or not found)'))

    // 2. 새 농장 딜레마 생성 (URL 인코딩된 경로)
    console.log('🌾 Creating new farm dilemma with URL-encoded paths...')
    ...
    const dilemma = await prisma.dilemma.create({
      data: {
        id: 'cmgzae36r0000hbb74fb5wv25', // Same ID
        title: '가뭄과 농장의 선택',
    ...
    console.log('\n✅ 농장 딜레마 재생성 완료!')
    console.log('📍 ID:', dilemma.id)
    console.log('🌾 Title:', dilemma.title)
    console.log('\n🔗 Test URL: http://localhost:3000/explore/' + dilemma.id)
